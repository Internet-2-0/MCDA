...
#pragma warning restore RECS0029 // Warns about property or indexer setters and event adders or removers that do not use the value parameter
        ...
        private Rectangle GetTabImageRect(Rectangle tabRect, GraphicsPath tabBorderPath)
        {
            ...
            if (this._StyleProvider.ShowTabCloser)
            {
                if (this.EffectiveRightToLeft)
                {
                    if (horizontalTabs && IsLeftAligned(imageAlignment)) imageRect.X += CustomTabControl.TabCloserButtonSize + 4;
                    if (!horizontalTabs && IsTopAligned(imageAlignment)) imageRect.Y += CustomTabControl.TabCloserButtonSize + 4;
                }
                else
                {
                    if (horizontalTabs && IsRightAligned(imageAlignment)) imageRect.X -= CustomTabControl.TabCloserButtonSize + 4;
                    if (!horizontalTabs && IsBottomAligned(imageAlignment)) imageRect.Y -= CustomTabControl.TabCloserButtonSize + 4;
                }
            }
