...
namespace MCDA_APP.Highlight.Patterns
{
    public class Definition
    {
        ...
        public string GetRegexPattern()
        {
            ...
            var regexPatterns = new StringBuilder();
            ...
            foreach (var pattern in Patterns.Values)
            {
                ...
                else if (pattern is RegexPattern)
                {
                    if (regexPatterns.Length > 1)
                    {
                        regexPatterns.Append("|");
                    }
                    regexPatterns.AppendFormat("(?'{0}'{1})", pattern.Name, pattern.GetRegexPattern());
                }
            }
