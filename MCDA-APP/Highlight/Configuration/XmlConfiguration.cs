...
using MCDA_APP.Highlight.Extensions;
...
namespace MCDA_APP.Highlight.Configuration
{
    public class XmlConfiguration : IConfiguration
    {
        ...
        private Pattern GetPattern(XElement patternElement)
        {
            ...
            if (patternType.Equals("regex", stringComparison))
            {
                return GetRegexPattern(patternElement);
            }

            throw new InvalidOperationException(String.Format("Unknown pattern type: {0}", patternType));
        }

        private RegexPattern GetRegexPattern(XElement patternElement)
        {
            var name = patternElement.GetAttributeValue("name");
            var regex = patternElement.GetAttributeValue("regex");
            var style = GetPatternStyle(patternElement);

            return new RegexPattern(name, style, regex);
            //var name = patternElement.GetAttributeValue("name");
            //var regex = patternElement.GetAttributeValue("regex");
            //var style = GetPatternStyle(patternElement);
        ...
        private SixLabors.Fonts.Font GetPatternFont(XElement fontElement, SixLabors.Fonts.Font defaultFont = null)
        {
            var fontFamily = fontElement.GetAttributeValue("name");
            if (fontFamily != null)
            {
                var emSize = fontElement.GetAttributeValue("size").ToSingle(11f);
                var style = Enum<SixLabors.Fonts.FontStyle>.Parse(fontElement.GetAttributeValue("style"), SixLabors.Fonts.FontStyle.Regular, true);

                return SixLabors.Fonts.SystemFonts.CreateFont(fontFamily, emSize, style);
            }
        ...
        private SixLabors.Fonts.Font GetDefinitionFont(XElement fontElement)
        {
            var fontName = fontElement.GetAttributeValue("name");
            var fontSize = Convert.ToSingle(fontElement.GetAttributeValue("size"));
            var fontStyle = (SixLabors.Fonts.FontStyle)Enum.Parse(typeof(SixLabors.Fonts.FontStyle), fontElement.GetAttributeValue("style"), true);

            return SixLabors.Fonts.SystemFonts.CreateFont(fontName, fontSize, fontStyle);
        }
    }
}
